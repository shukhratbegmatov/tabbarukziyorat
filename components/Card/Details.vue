<template>
  <div
    class="relative -mt-[78px] z-30 mb-8 sm:mb-12 md:mb-16"
    v-if="single?.populations"
  >
    <div>
      <div class="container xl:max-w-fit">
        <div
          class="flex flex-col lg:flex-row items-center lg:items-start xl:justify-start justify-center gap-4 xl:gap-6"
        >
          <div
            v-if="single?.attractions_count"
            class="p-4 md:pr-6 xl:pr-[73px] bg-white/10 border border-white/10 rounded-xl backdrop-blur-[12px]"
          >
            <div class="flex items-center gap-x-3 mb-4">
              <div class="bg-red flex-center w-9 h-9 rounded-lg">
                <i class="icon-mosque text-white"></i>
              </div>

              <p
                class="text-white font-semibold text-lg md:text-xl xl:text-2xl leading-130"
              >
                {{ single?.attractions_count }}
              </p>
            </div>
            <p
              class="font-medium text-lg md:text-xl xl:text-2xl leading-130 text-white opacity-60"
            >
              {{ $t('attractions') }}
            </p>
          </div>
          <div
            class="p-4 bg-white/10 border border-white/10 rounded-xl backdrop-blur-[12px] grid gap-5"
            v-if="single?.populations?.length"
            :class="[
              single?.populations?.length
                ? ' grid-cols-2 sm:grid-cols-4'
                : `grid-cols-${single?.populations?.length}`,
            ]"
          >
            <div
              class="detail-wrapper relative pr-[22px]"
              v-if="single?.populations?.length"
              :class="{ 'after:hidden': !single?.populations?.length }"
            >
              <div class="flex items-center gap-x-3 mb-4">
                <div class="w-9 h-9 rounded-lg flex-center bg-red">
                  <svg
                    width="36"
                    height="36"
                    viewBox="0 0 36 36"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                  >
                    <rect
                      width="36"
                      height="36"
                      rx="8"
                      transform="matrix(-1 0 0 1 36 0)"
                      fill="#E64545"
                    />
                    <mask
                      id="mask0_197_21567"
                      style="mask-type: alpha"
                      maskUnits="userSpaceOnUse"
                      x="6"
                      y="7"
                      width="24"
                      height="23"
                    >
                      <rect
                        x="6"
                        y="7"
                        width="24.0004"
                        height="22.7741"
                        fill="url(#pattern0)"
                      />
                    </mask>
                    <g mask="url(#mask0_197_21567)">
                      <rect
                        x="4.77344"
                        y="8.22629"
                        width="25.1951"
                        height="20.3277"
                        fill="white"
                      />
                    </g>
                    <defs>
                      <pattern
                        id="pattern0"
                        patternContentUnits="objectBoundingBox"
                        width="1"
                        height="1"
                      >
                        <use
                          xlink:href="#image0_197_21567"
                          transform="matrix(0.00184821 0 0 0.00195312 0.0227273 0)"
                        />
                      </pattern>
                      <image
                        id="image0_197_21567"
                        width="512"
                        height="512"
                        xlink:href="data:image/png;base64,"
                      />
                    </defs>
                  </svg>
                </div>

                <p
                  class="text-white font-semibold text-lg md:text-xl xl:text-2xl leading-130"
                >
                  {{ nFormatter(single?.population) }}
                </p>
              </div>
              <p
                class="font-medium text-lg md:text-xl xl:text-2xl leading-130 text-white opacity-60"
              >
                {{ $t('population') }}
              </p>
            </div>
            <div
              class="detail-wrapper relative pr-4 md:pr-6 xl:pr-[87px]"
              v-if="third?.percentage"
            >
              <div class="flex items-center gap-x-3 mb-4">
                <div
                  class="w-9 h-9 rounded-lg flex-center bg-white/10 flex-shrink-0"
                >
                  <img
                    class="w-[18px] h-[18px]"
                    :src="third?.religion?.flag"
                    alt=""
                  />
                </div>

                <p
                  class="text-white font-semibold text-lg md:text-xl xl:text-2xl leading-130"
                >
                  {{ third?.percentage }}%
                </p>
              </div>
              <p
                class="font-medium text-lg md:text-xl xl:text-2xl leading-130 text-white opacity-60"
              >
                {{ third?.religion?.title }}
              </p>
            </div>
            <div
              class="detail-wrapper relative pr-4 md:pr-6"
              v-if="second?.percentage"
            >
              <div class="flex items-center gap-x-3 mb-4">
                <div
                  class="w-9 h-9 rounded-lg flex-center bg-white/10 flex-shrink-0"
                >
                  <img
                    class="w-[18px] h-[18px]"
                    :src="second?.religion?.flag"
                    alt=""
                  />
                </div>

                <p
                  class="text-white font-semibold text-lg md:text-xl xl:text-2xl leading-130"
                >
                  {{ second?.percentage }}%
                </p>
              </div>
              <p
                class="font-medium text-lg md:text-xl xl:text-2xl leading-130 text-white opacity-60"
              >
                {{ second?.religion?.title }}
              </p>
            </div>
            <div
              class="detail-wrapper after:opacity-0 relative pr-4 md:pr-10"
              v-if="first?.percentage"
            >
              <div class="flex items-center gap-x-3 mb-4">
                <div
                  class="w-9 h-9 rounded-lg flex-center bg-white/10 flex-shrink-0"
                >
                  <img
                    class="w-[18px] h-[18px]"
                    :src="first?.religion?.flag"
                    alt=""
                  />
                </div>

                <p
                  class="text-white font-semibold text-lg md:text-xl xl:text-2xl leading-130"
                >
                  {{ first?.percentage }}%
                </p>
              </div>
              <p
                class="font-medium text-lg md:text-xl xl:text-2xl leading-130 text-white opacity-60"
              >
                {{ first?.religion?.title }}
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { nFormatter } from '../../helpers'
import { computed } from 'vue'

interface Props {
  single: object
}

const props = defineProps<Props>()

const first = computed(() => {
  return props?.single?.populations[0]
})
const second = computed(() => {
  return props?.single?.populations[1]
})
const third = computed(() => {
  return props?.single?.populations[2]
})
</script>

<style scoped>
.detail-wrapper::after {
  content: '';
  position: absolute;
  right: 0;
  top: 0;
  width: 1px;
  height: 100%;
  background: linear-gradient(
    180deg,
    rgba(255, 255, 255, 0) 0%,
    #ffffff 51.04%,
    rgba(255, 255, 255, 0) 100%
  );
}

@media (max-width: 768px) {
  .detail-wrapper:nth-child(even)::after {
    opacity: 0;
  }
}
</style>
