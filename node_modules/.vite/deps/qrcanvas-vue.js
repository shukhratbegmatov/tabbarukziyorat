...
// node_modules/qrcanvas/lib/qrcanvas.esm.js
...
function drawCanvas(canvas, data, options) {
  ...
  var _options = options, cellSize = _options.cellSize, context = _options.context, _options$clear = _options.clear, clear = _options$clear === void 0 ? true : _options$clear;
  ...
  while (queue.length) {
    ...
    if (Array.isArray(item)) {
      ...
      var h2 = ("rows" in obj ? obj.rows * cellSize : obj.h) || width;
      if (obj.image) {
        ctx.drawImage(obj.image, x, y, w, h2);
      } else {
...
function drawText(text, options) {
  var _ref = options || {}, _ref$fontSize = _ref.fontSize, fontSize = _ref$fontSize === void 0 ? 64 : _ref$fontSize, _ref$fontStyle = _ref.fontStyle, fontStyle = _ref$fontStyle === void 0 ? "" : _ref$fontStyle, _ref$fontFamily = _ref.fontFamily, fontFamily = _ref$fontFamily === void 0 ? "Cursive" : _ref$fontFamily, _ref$color = _ref.color, color = _ref$color === void 0 ? null : _ref$color, _ref$pad = _ref.pad, pad = _ref$pad === void 0 ? 8 : _ref$pad, _ref$padColor = _ref.padColor, padColor = _ref$padColor === void 0 ? COLOR_WHITE : _ref$padColor, _ref$mode = _ref.mode, mode = _ref$mode === void 0 ? 1 : _ref$mode;
  ...
  var resetContext = function resetContext2() {
  ...
  canvas.height = height;
  resetContext();
  var fillText = function fillText2() {
  ...
  if (mode === 1) {
    ...
    if (padColorArr) {
      (function() {
        ...
        var padded = [];
        var offset = 0;
        var _loop = function _loop2(loop2) {
          var current = [];
          var unique = {};
          ...
          last.forEach(function(i2) {
            if (data[4 * i2 + 3]) {
              [i2 % width ? i2 - 1 : -1, (i2 + 1) % width ? i2 + 1 : -1, i2 - width, i2 + width].forEach(function(j) {
                var k = 4 * j;
...
function renderDefault(_ref) {
  var foreground = _ref.foreground, cellSize = _ref.cellSize, isDark = _ref.isDark, count = _ref.count;
  var getCanvas2 = helpers.getCanvas, drawCanvas2 = helpers.drawCanvas;
  var width = cellSize * count;
  var canvasMask = getCanvas2(width);
  ...
  drawCells({
    ...
    var i = _ref2.i, j = _ref2.j, x = _ref2.x, y = _ref2.y;
    if (isDark(i, j)) {
      context.fillRect(x, y, cellSize, cellSize);
    }
  });
  var canvasFg = getCanvas2(width);
  drawCanvas2(canvasFg, foreground, {
    cellSize
  });
...
function renderRound(_ref3, maskOptions) {
  var foreground = _ref3.foreground, cellSize = _ref3.cellSize, isDark = _ref3.isDark, count = _ref3.count;
  var getCanvas2 = helpers.getCanvas, drawCanvas2 = helpers.drawCanvas;
  var width = cellSize * count;
  var canvasMask = getCanvas2(width);
  var _maskOptions$value = maskOptions.value, value = _maskOptions$value === void 0 ? 1 : _maskOptions$value;
  ...
  drawCells({
    ...
    var i = _ref4.i, j = _ref4.j, x = _ref4.x, y = _ref4.y;
    if (isDark(i, j)) {
      context.beginPath();
...
function renderFusion(_ref5, maskOptions) {
  var foreground = _ref5.foreground, cellSize = _ref5.cellSize, isDark = _ref5.isDark, count = _ref5.count;
  var getCanvas2 = helpers.getCanvas, drawCanvas2 = helpers.drawCanvas;
  var width = cellSize * count;
  var canvasMask = getCanvas2(width);
  var context = canvasMask.getContext("2d");
  context.fillStyle = COLOR_BLACK;
  var _maskOptions$value2 = maskOptions.value, value = _maskOptions$value2 === void 0 ? 1 : _maskOptions$value2;
  var radius = value * cellSize / 2;
  drawCells({
    cellSize,
    count
  }, function(_ref6) {
    var i = _ref6.i, j = _ref6.j, x = _ref6.x, y = _ref6.y;
    var corners = [0, 0, 0, 0];
    if (isDark(i - 1, j)) {
      corners[0] += 1;
      corners[1] += 1;
...
function renderSpot(_ref7, maskOptions) {
  var foreground = _ref7.foreground, cellSize = _ref7.cellSize, isDark = _ref7.isDark, count = _ref7.count;
  var getCanvas2 = helpers.getCanvas, drawCanvas2 = helpers.drawCanvas;
  var width = cellSize * count;
  var canvasMask = getCanvas2(width);
  var value = maskOptions.value, _maskOptions$foregrou = maskOptions.foregroundLight, foregroundLight = _maskOptions$foregrou === void 0 ? COLOR_WHITE : _maskOptions$foregrou;
  var context = canvasMask.getContext("2d");
  var canvasLayer = getCanvas2(width);
  var canvasFg = getCanvas2(width);
  ...
  [{
    ...
    drawCells({
      ...
      var i = _ref8.i, j = _ref8.j, x = _ref8.x, y = _ref8.y;
...
function drawCells(_ref9, drawEach) {
  var cellSize = _ref9.cellSize, count = _ref9.count;
...
var QRCanvasRenderer = function() {
  function QRCanvasRenderer2(options) {
    var _this = this;
    this.options = _extends({}, DEFAULTS);
    this.cache = {};
    this.isDark = function(i, j) {
      var _this$cache = _this.cache, qr = _this$cache.qr, count = _this$cache.count;
  ...
  var _proto = QRCanvasRenderer2.prototype;
  _proto.render = function render(canvas, config) {
    if (config === void 0) {
      config = {};
    }
    var _this$options = this.options, background = _this$options.background, foreground = _this$options.foreground, padding = _this$options.padding, effect = _this$options.effect, logo = _this$options.logo, resize = _this$options.resize;
    var onRender = effects[effect.type] || effects["default"];
    var count = this.cache.count;
    var drawCanvas2 = helpers.drawCanvas;
    var _config = config, size = _config.size;
    var canvasOut;
    var canvasBg;
    var canvasFg;
    {
      var _config2 = config, cellSize = _config2.cellSize;
      if (!canvas && !cellSize && !size)
        cellSize = 6;
      if (cellSize)
        size = count * cellSize + padding + padding;
      if (size) {
    ...
    var contentSize = size - padding - padding;
    {
      ...
      drawCanvas2(canvasBg, background, {
        cellSize: _cellSize
      });
      canvasFg = onRender(_extends({
        ...
        cellSize: _cellSize,
        isDark: this.isDark
      }, this.cache), this.options.effect);
      if (logo) {
        var logoLayer = _extends({}, logo);
        if (!logo.w && !logo.h && !logo.cols && !logo.rows) {
          var _ref = logo.image, width = _ref.width, height = _ref.height;
          var imageRatio = width / height;
          var posRatio = Math.min((count - 18) / count, 0.38);
          var h2 = Math.min(height, sketchSize * posRatio, sketchSize * posRatio / imageRatio);
          var w = h2 * imageRatio;
          var x = (sketchSize - w) / 2;
          var y = (sketchSize - h2) / 2;
          logoLayer.w = w;
          logoLayer.h = h2;
          logoLayer.x = x;
          logoLayer.y = y;
        }
        drawCanvas2(canvasFg, logoLayer, {
          clear: false
        });
      }
    }
    drawCanvas2(canvasOut, [{
      image: canvasBg
    }, {
      image: canvasFg,
      x: padding,
  ...
  _proto.setOptions = function setOptions(options) {
    ...
    this.normalizeLogo();
    var _this$options2 = this.options, typeNumber = _this$options2.typeNumber, data = _this$options2.data, logo = _this$options2.logo;
    var correctLevel = this.options.correctLevel;
    if (logo && ["Q", "H"].indexOf(correctLevel) < 0)
      correctLevel = "H";
    var qr = (0, import_qrcode_generator.default)(typeNumber, correctLevel);
    qr.addData(data || "");
    qr.make();
  ...
  _proto.normalizeLogo = function normalizeLogo() {
    var isDrawable2 = helpers.isDrawable, drawText2 = helpers.drawText;
    ...
    if (logo) {
      if (isDrawable2(logo)) {
      ...
      } else if (!isDrawable2(logo.image)) {
        ...
        if (typeof logo.text === "string") {
          logo = {
            image: drawText2(logo.text, logo.options)
          };
        } else {
          logo = null;
        }
  ...
  return QRCanvasRenderer2;
}();
function qrcanvas(options) {
  var canvas = options.canvas, size = options.size, cellSize = options.cellSize, rest = _objectWithoutPropertiesLoose(options, ["canvas", "size", "cellSize"]);
...
// node_modules/qrcanvas-vue/dist/qrcanvas-vue.esm.js
...
var QRCanvas = defineComponent({
  ...
  setup: function setup(props, context) {
    ...
    var update = function update2() {
      var options = _extends2({}, props.options, {
        canvas: canvas.value
      });
      if (!canvas.value)
        return;
    ...
    return function() {
      props.options;
      var rest = _objectWithoutPropertiesLoose2(props, _excluded);
      return h("canvas", _extends2({}, rest, {
        ref: canvas
      }));
    };
  }
...
qrcanvas/lib/qrcanvas.esm.js:
  (*! qrcanvas v3.1.2 | ISC License *)

qrcanvas-vue/dist/qrcanvas-vue.esm.js:
  (*! qrcanvas-vue v3.0.0 | ISC License *)
*/
//# sourceMappingURL=qrcanvas-vue.js.map
